    ...
    <section>
        ...
        <el-form :model="form" ref="form" :rules="FormRules">
            <div style="border: 1px solid #e6e6e6;margin-bottom: 10px;padding:15px">
                ...
                <el-row :gutter="10">
                    <el-col :span='8'>
                        <el-form-item label="主文件名:" label-width="83px" prop="modelFileValue">
                            <el-select v-model="form.modelFileValue" filterable remote reserve-keyword
                                       style="display: block; user-select: none;"
                                       @change="changeOption" 
                                       placeholder="模型主程序文件名"
                                       :remote-method="remoteMethod"
                                       :loading="loading">
                                <el-option v-for="item in modelFileOptions" 
                                           :key="item.value"
                                           :label="item.label"
                                           :value="item.value">
...
<script>
    import { addApiDetail, getApiGroupList, getModelReposAPI } from "../../../api/api";
        ...
        data() {
            return {
                ...
                FormRules: {
                    ...
                    modelFileValue:[{ required: true, message: '请输入主程序文件名', trigger: 'blur' }],
        ...
        methods: {
            remoteMethod(query) {
                if (query !== '') {
                    ...
                    setTimeout(() => {
                        this.loading = false;
                        this.modelFileOptions = this.modelFileList.filter(item => {
                            return item.label.toLowerCase()
                                .indexOf(query.toLowerCase()) > -1;
                        });
                    }, 200);
                } else {
                    this.modelFileOptions = [];
                }
            },
            changeOption(){
                console.log(this.modelFileValue,this.modelFileValue.length)
                let obj = {}
                obj = this.modelFileOptions.find(
                    item=>{
                        
                        return item.ProcessNode=this.value //筛选出对应数据
                    }
                )
               this.sels = this.form.modelFileValue;
            },
            ...
            getModelRepos() {
                ...
                getModelReposAPI(headers, params).then(_data => {
                    let {msg, code, data} = _data;
                    self.listLoading = false;
                    if (code === '999999') {
                        self.modelFileData = data.modelFileList;
                        self.workspacePath = data.workspacePath;
                        //self.modelFileData = data.modelFileMap;
                        this.modelFileList = data.modelFileList.map(item => {
                            console.log(item,"itemmmmmm")
                            return { value: `${item.filevalue}`, label: `${item.filekey}`};
                        });

                        
                        console.log(self.modelFileData,"self.modelDirList")
            ...
            addApi: function () {
                let modelRelpathFile = this.sels;
                    ...
                    if (valid) {
                        ...
                        this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            let params = {
                                ...
                                mainFile: self.form.modelFileValue,//self.form.mainFile,
                                mainFilePath: self.workspacePath,//self.form.mainFilePath,
                                //status: self.form.status,
                                metrics: {"auc":0.99},//self.form.metrics,
