<template>
	<section>
		...
		<!--列表-->
		<el-table :data="api" highlight-current-row v-loading="listLoading" 
			...
			<el-table-column prop="name" label="服务名称" min-width="17%" sortable show-overflow-tooltip>
				<template slot-scope="scope">
					<el-icon name="name"></el-icon>
					<router-link :to="{ name: '基础信息', params: {api_id: scope.row.id}}" style='text-decoration: none;'>{{ scope.row.name }}</router-link>
				</template>
			</el-table-column>
			<el-table-column prop="version" label="服务版本" min-width="11%" sortable show-overflow-tooltip>
			</el-table-column>
			<el-table-column prop="servingIP" label="服务地址" min-width="19%" sortable show-overflow-tooltip>
			</el-table-column>
			<el-table-column prop="metrics" label="模型指标" min-width="13%" sortable show-overflow-tooltip>
			</el-table-column>
			<el-table-column prop="LastUpdateTime":formatter="dateFormat" label="更新日期" min-width="15%" sortable show-overflow-tooltip>
			</el-table-column>
			<el-table-column label="服务操作" min-width="7%">
				<template slot-scope="scope">
                    <!--<el-button type="info" size="small" @click="handleChangeStatus(scope.$index, scope.row)">{{scope.row.status===false?'上线':'下线'}}</el-button>-->
					<el-button v-if="scope.row.status" type="success" size="small" @click="handleChangeStatus(scope.$index,scope.row)">下线</el-button>
					<el-button v-if="!scope.row.status" type="info" size="small"  @click="handleChangeStatus(scope.$index,scope.row)">上线</el-button>
				</template>
			</el-table-column>
			<el-table-column label="操作" min-width="13%">
				<template slot-scope="scope">
					<el-button type="danger" size="small" @click="handleDel(scope.$index, scope.row)">删除</el-button> 
					<!--<router-link :to="{ name: '新增接口', params: {api_id: scope.row.id}}" style='text-decoration: none;color: aliceblue;'>
						<el-button type="info" size="small" style="margin-left:10px;">修改</el-button>
					</router-link>-->
				</template>
			</el-table-column>
		</el-table>
...
<script>
    import { test,hostModel } from '../../../api/api'
    import $ from 'jquery'
    ...
    export default {
        ...
        methods: {
            ...
            dateFormat:function(row, column) { //日期格式化
        ...
        handleChangeStatus: function(index, row) {
                ...
                if (row.status) {
                    hostModel(headers, params).then(_data => {
            ...
            getApiList() {
                ...
                $.ajax({
                    type: "get",
                    url: test+"/api/serving/service_total",
                    async: true,
                    data: param,
                    headers: {
                        Authorization: 'Token '+JSON.parse(sessionStorage.getItem('token'))
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.code === '999999') {
                                ...
                                self.$router.push({ name: '分组服务列表', params: { project_id: self.$route.params.project_id, firstGroup: self.updateGroupForm.firstGroup}});
                            }
                            else {
                                self.$message.error({
                                    message: data.msg,
