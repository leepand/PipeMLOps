""" A flask application for hosting machine learning models. """
import os
from flask import Flask
from flask_cors import CORS
from flask_httpauth import HTTPBasicAuth

from stock_service.config import config
from stock_service.models.model_function import db

from playhouse.flask_utils import FlaskDB


config_name = os.getenv('FLASK_CONFIG') or 'default'
basedir = os.path.abspath(os.path.dirname(__file__))

app = Flask(__name__)
FlaskDB(app, db) #解决peewee不自动关闭连接池连接，参见https://www.cnblogs.com/xueweihan/p/6698456.html

# r'/*' 是通配符，让本服务器所有的 URL 都允许跨域请求
CORS(app, resources=r'/*')
CORS(app, supports_credentials=True)
app.config['SECRET_KEY'] = 'hard to guess string'

app.config.from_object(config[config_name])


auth = HTTPBasicAuth()
CSRF_ENABLED = True
app.debug = True


import  stock_service.apps.login
import  stock_service.apps.data_center
import  stock_service.apps.strategy