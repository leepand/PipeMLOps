...
.PHONY: help venv start-prod dependencies test-dependencies abtesting clean-pyc test start-server test-models-endpoint test-metadata-endpoint test-predict-endpoint
...
dependencies: ## install dependencies from requirements.txt
	tar xvf modelcloud-0.2.0.tar.gz &&     pip install ./modelcloud-0.2.0/ -i https://pypi.tuna.tsinghua.edu.cn/simple &&  pip install -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple && pip install ./bolt4ds/ -i https://pypi.tuna.tsinghua.edu.cn/simple
...
start-server: ## start the local development server
	export APP_SETTINGS="stock_service.config.DevelopmentConfig"; \
	export FLASK_APP=stock_service; \
	export FLASK_DEBUG=1; \
    export FLASK_RUN_HOST="127.0.0.1"; \
    export FLASK_RUN_PORT=6002; \
	flask run
start-prod: ## start the local production server
	gunicorn --workers=3 -b 0.0.0.0:6001  model_service.wsgi:app >>web-predict.log;
